...
def benchmark_storage_scaling(U_values=[100, 500, 1000, 2000],
    ...
    for U in U_values:
        ...
        print(f"  单个签名大小: {storage_info['sigma_size_bytes']} 字节")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--benchmark-type", type=str,
                        choices=["all", "pwgen", "verify", "phi", "U", "storage", "storage-scaling"], default="all",
                        help="性能测试类型：all-全部, pwgen-密码生成, verify-验证, phi-子集数量影响, U-成员数量影响,storage-存储数据量, storage-scaling-存储扩展性")
